...
static int 	check_intersection(t_list **objs, t_object *obj, t_vector *cucn, t_coll *coll)
{
	int i;
...
static int 	get_intersections(double *res, const t_vector *untr_od, t_vector *od, t_object *obj)
{
	t_torus		*trs;
	double 		doxr[4];
	double 		abcde[5];
...
float		ft_collide_torus(t_list **objs, struct s_object *obj, t_coll *coll, t_vector untr_od[2])
{
	...
	if ((i[2] = get_intersections(res, untr_od, od, obj)) == 0)
		return (FLT_MAX);
	i[0] = -1;
	i[1] = 0;
	t = FLT_MAX;
...
int			ft_is_inside_torus(t_object *o, t_vector point)
{
	...
	if (r[1] > trs->r_outer2 + trs->r_inner2 + 2 * trs->r_outer * trs->r_inner ||
		r[0] > trs->r_inner2)
		return (0);
	return (1);
}
